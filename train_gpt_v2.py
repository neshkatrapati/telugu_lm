...
def build_model(config: GPTConfig, device: str = "cuda"):
    ...
    class CausalSelfAttention(nn.Module):
        def __init__(self, config):
            ...
            # Base v2: QK-norm (Llama 3.1 / Cosmos) — RMSNorm on Q and K before RoPE
        ...
        def forward(self, x, freqs_cis):
            ...
            # Base v2: QK-norm — RMSNorm on head_dim before RoPE
            if self.use_qk_norm:
                q = self.q_norm(q)
                k = self.k_norm(k)
    ...
    class GPT(nn.Module):
        def __init__(self, config):
            ...
            # Weight tying (default: tied; Base v2: untied for extra lm_head capacity)
        ...
        def forward(self, idx, targets=None, pattern_ids=None):
            ...
            if targets is not None:
                ...
                # Base v2: z-loss — λ × E[log(sum(exp(logits)))²], prevents logit drift
  ...
  %(prog)s train --data ./train_data --tokenizer ./tokenizer --resume ./checkpoints/step_005000.pt
    ...
    def _add_train_args(p):
        ...
        p.add_argument("--block-size", type=int, default=2048,
                       help="Context length (default: 2048; Base v2: 4096)")
        p.add_argument("--rope-theta", type=float, default=10000.0,
                       help="RoPE base frequency (default: 10000; Base v2 long-context: 500000)")
        p.add_argument("--warmup-steps", type=int, default=500,
                       help="LR warmup steps (default: 500; Base v2: 3000)")
        p.add_argument("--min-lr", type=float, default=None,
                       help="Minimum LR for cosine/wsd decay (default: lr/10)")
        p.add_argument("--untie-embeddings", action="store_true",
                       help="Untie lm_head from wte (Base v2; +36M params @ vocab=47831)")
        p.add_argument("--use-z-loss", action="store_true",
                       help="Add z-loss term to prevent logit drift (Base v2)")
        p.add_argument("--z-loss-weight", type=float, default=1e-4,
                       help="Z-loss coefficient (default: 1e-4)")
        p.add_argument("--use-qk-norm", action="store_true",
                       help="Apply QK-norm in attention (Base v2, Llama 3.1 style)")
    ...
    def _apply_train_args(args, model_config, train_config):
        ...
        model_config.tie_embeddings = not args.untie_embeddings
        model_config.use_z_loss = args.use_z_loss
        model_config.z_loss_weight = args.z_loss_weight
        model_config.use_qk_norm = args.use_qk_norm
        train_config.warmup_steps = args.warmup_steps
        if args.min_lr is not None:
            train_config.min_lr = args.min_lr
        else:
            train_config.min_lr = args.lr / 10.0
